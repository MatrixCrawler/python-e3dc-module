class RSCPLib:
    ...
    def get_data_type_name(self, type_hex):
        """
        This method will determine the data field type based on the hex code
        :param type_hex:
        :return: The data type as string
        """
        try:
            return self._rscp_data_types[type_hex]
        except KeyError:
            return hex(type_hex)

    def get_data_type_hex(self, type_name):
        try:
            return self._rscp_data_types.keys()[self._rscp_data_types.values().index(type_name)]
        except KeyError:
            return int(type_name, 0)

    def get_data_tag_name(self, tag_hex):
        try:
            return self.rscp_tags[tag_hex]
        except KeyError:
            return hex(tag_hex)

    def get_hex_code(self, tag_name):
        try:
            return self.rscp_tags.keys()[self.rscp_tags.values().index(tag_name)]
        except KeyError:
            return int(tag_name, 0)
